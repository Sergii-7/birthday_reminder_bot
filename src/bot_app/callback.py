...
from src.bot_app.dir_menu.menu import AdminMenu, Menu, SetChat, SetEvent
        ...
        if data.startswith("user"):
            ...
            if data == "1":
                ...
                for text in text_list:
                    ...
                    text = (
                        f"<b>📅 Календар подій 📅</b>\n ------------\n{text.strip()}" if text == text_list[0] else text
                    )
            ...
            if user.info in ["super-admin", "admin"] or telegram_id == sb_telegram_id:
                ...
                if data == ":m" or "_set_chat_" in data:
                    ...
                    if chat and chat.status:
                        await SetChat().get_command(
                            user=user, chat=chat, command=command, callback_query=callback_query
                        )
                    else:
