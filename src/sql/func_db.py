...
@retry_on_db_error()
async def get_doc_by_id(model: str, doc_id: int) -> Optional[ModelType]:
...
@retry_on_db_error()
async def doc_update(doc: ModelType) -> ModelType:
...
async def create_new_doc(model: str, data: dict, data_has_datatime: bool = False) -> Optional[int]:
    ...
    logger.debug(f"model_name: {model}, data: {data}, data_has_datatime: {data_has_datatime}")
    if model not in models:
        logger.error(f"Invalid model name provided: {model}")
        return None
...
@retry_on_db_error()
async def get_all_docs(model: str) -> List[ModelType]:
    """Get all docs from DataBase from table 'model'."""
    if model not in models:
        logger.error(f"model: {model} is invalid!")
        return []
    model: Type[ModelType] = models[model]
...
async def _demo() -> None:
    ...
    if user:
        print(f"User found: {user.first_name} {user.last_name}, Username: {user.username}")
    else:
